...
export const Route = createFileRoute("/app/startup/community")({
  component: CommunitySectionWrapper,
});
        ...
        <AnimatePresence mode="wait">
          ...
          {activeSubTab === 'discuss' && (
            ...
            <motion.div 
              ...
              className="space-y-6"
            >
              <div className="flex justify-between items-center bg-[#0B0F19] border border-[#1F2947] p-4 rounded-2xl">
                <span className="text-xs font-semibold text-slate-300">Struggling with SaaS models? Converse with matching co-founders.</span>
                <button
                ...
                >
                  <form onSubmit={handleCreatePost} className="space-y-3">
                    ...
                    <textarea
                      ...
                      className="w-full text-xs p-3 rounded-xl border border-[#1F2947] bg-[#0B0F19] h-24 resize-none"
                    />
                    ...
                    <div className="space-y-3 flex-1">
                      <div className="flex items-center gap-2.5">
                        ...
                        <div className="text-[10px] space-y-0.5">
                          <span className="font-extrabold text-white block leading-none">{post.author}</span>
                          <span className="text-slate-400 block leading-none">{post.role}</span>
                        </div>
                      </div>

                      <h4 className="text-xs font-bold text-slate-850 text-white leading-snug">{post.title}</h4>
                      <p className="text-[11px] text-slate-400 leading-relaxed">{post.content}</p>
                    ...
                    <div className="space-y-3 font-sans">
                      <div className="flex gap-3 items-center">
                        ...
                        <div className="text-xs">
                          <h4 className="font-extrabold text-slate-850 text-white leading-none">{profile.name}</h4>
                          <span className="text-indigo-600 dark:text-indigo-400 font-semibold block pt-0.5">{profile.role}</span>
                        </div>
                      </div>

                      <div className="bg-[#0B0F19] border border-[#1F2947] p-3 rounded-xl divide-y divide-slate-100 dark:divide-slate-850 text-[10px] space-y-2">
                        <div className="flex justify-between pb-1.5 text-slate-450 font-medium">
                          <span>Startup:</span>
                          <span className="font-bold text-slate-300">{profile.stage}</span>
                        </div>
                        <div className="flex justify-between pt-1.5 text-slate-450 font-medium font-medium">
                          <span>Seeking Co-Founders:</span>
                          <span className="font-bold text-slate-300">{profile.seeking}</span>
                        </div>
                      </div>
                      ...
                      <div className="space-y-1.5 pt-1">
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {profile.skills.map((skill, idx) => (
                            <span key={idx} className="bg-[#0B0F19] dark:bg-slate-850 text-slate-600 dark:text-slate-350 text-[9px] px-2 py-0.5 rounded-md font-mono">{skill}</span>
                          ))}
                        </div>
                      </div>
                    </div>
          ...
          {activeSubTab === 'events' && (
            ...
            >
              {STARTUP_EVENTS.map((ev) => {
                ...
                return (
                  <div key={ev.id} className="bg-[#0F1528] border border-[#1F2947] p-5 rounded-2xl flex flex-col md:flex-row justify-between items-start md:items-center gap-4 shadow-xs">
          ...
          {activeSubTab === 'leaderboard' && (
            ...
            <motion.div 
              ...
              className="bg-[#0F1528] border border-[#1F2947] rounded-2xl overflow-hidden shadow-sm font-sans"
            >
              <div className="p-4 bg-[#0B0F19] border-b border-slate-150 dark:border-slate-800 flex justify-between items-center text-xs">
                  ...
                  <div key={user.rank} className="p-4 flex items-center justify-between text-xs hover:bg-[#1F2947]/40">
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        />
                        <div className="text-xs">
                          <span className="font-extrabold text-white block leading-none">{user.name}</span>
                    ...
                    <div className="flex items-center gap-6 font-mono text-xs">
                      <div className="text-right">
                        <span className="block font-bold text-white">{user.badgeCount}</span>
...
function CommunitySectionWrapper() {
  const context = useStartupContext();
  return <CommunitySection {...context} />;
}
