...
export const Route = createFileRoute("/app/startup/")({
  component: HomeDashboardWrapper,
});
    ...
    <div id="home-dashboard" className="space-y-8 max-w-7xl mx-auto px-1">
      ...
      {score === 0 ? (
        ...
        <motion.div 
          ...
          animate={{ opacity: 1, y: 0 }}
          className="bg-[#0F1528] border border-[#1F2947] p-8 rounded-3xl text-center space-y-6 shadow-sm"
        >
          <div className="w-16 h-16 bg-indigo-500/10 rounded-2xl flex items-center justify-center mx-auto text-indigo-600 dark:text-indigo-400">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-5 bg-[#0F1528] border border-[#1F2947] p-6 rounded-3xl shadow-sm flex flex-col justify-between space-y-6 animate-pulse-slow">
            <div className="flex justify-between items-start">
              <div className="space-y-0.5">
                ...
                <h3 className="font-display font-extrabold text-slate-200 text-lg">Tracks Vitals Radar</h3>
              </div>
          ...
          <div className="lg:col-span-7 bg-[#0F1528] border border-[#1F2947] p-6 rounded-3xl shadow-sm flex flex-col justify-between h-full space-y-6">
            <div className="space-y-4">
              <div className="flex items-center gap-2 text-indigo-650 dark:text-indigo-400">
                <Target className="w-5 h-5" />
                <h3 className="font-display font-extrabold text-slate-100">Tactical Focus Assistant</h3>
              </div>

              {/* Dynamic advisories change based on concentric selector */}
              <div className="bg-[#0B0F19] border border-[#1F2947] p-4 rounded-2xl flex flex-col justify-between gap-2.5">
                <div>
                  <div className="flex justify-between items-center mb-1">
                    <span className="font-display font-black text-xs text-slate-100">
              ...
              <div className="space-y-3">
                ...
                {focusTrackData[activeFocusTrack].tactics.map((tactic, idx) => (
                  <div 
                    key={idx} 
                    className="flex gap-3 bg-brand-50 hover:-translate-y-0.5 transition-transform bg-[#0B0F19]/60 p-3 rounded-2xl border border-[#1F2947] font-sans"
                  >
                    <div className="w-6 h-6 rounded-full bg-[#0B0F19] dark:bg-slate-850 font-bold text-[10px] text-slate-650 dark:text-slate-350 flex items-center justify-center shrink-0">
                      {idx + 1}
                    </div>
                    <div className="text-left space-y-0.5">
                      <div className="flex items-center gap-2">
                        <span className="text-xs font-bold text-slate-100 leading-none">{tactic.label}</span>
            ...
            <div className="pt-4 border-t border-[#1F2947] dark:border-slate-855 flex justify-between items-center text-xs">
          ...
          <div className="lg:col-span-6 bg-[#0F1528] border border-[#1F2947] p-6 rounded-3xl shadow-sm">
            ...
            <div className="space-y-3">
              {strengths.map((str, idx) => (
                <div key={idx} className="flex items-center justify-between border-b last:border-0 border-[#1F2947] pb-3 last:pb-0 font-sans font-medium">
                  <div className="flex items-center gap-2.5 truncate">
                    <CheckCircle2 className="w-4 h-4 text-emerald-500 shrink-0" />
                    <span className="text-xs font-semibold text-slate-300 truncate">{str.name}</span>
                  </div>
                  
                  <div className="flex items-center gap-2 shrink-0">
                    <div className="w-16 bg-[#1F2947] h-1.5 rounded-full overflow-hidden">
              ...
              {risks.map((risk, idx) => (
                <div key={idx} className="flex items-center justify-between border-b last:border-0 border-[#1F2947] pb-3 last:pb-0 font-sans font-medium">
                  <div className="flex items-center gap-2.5 truncate">
                    ...
                    <span className="text-xs font-semibold text-slate-300 truncate">{risk.name}</span>
                  </div>
          ...
          <div className="lg:col-span-4 bg-[#0F1528] border border-[#1F2947] p-5 rounded-3xl shadow-sm relative overflow-hidden h-full flex flex-col justify-between">
            <div className="space-y-2">
              ...
              <h4 className="text-xs font-extrabold font-display text-slate-200">Roadmap Progress</h4>
            ...
            <div className="mt-4 pt-4 border-t border-slate-50 dark:border-slate-850">
              <div className="flex justify-between text-[11px] text-slate-500 mb-2">
                ...
                <span className="font-bold text-slate-200">{completedMilestones}/{totalMilestones}</span>
              </div>
              <button 
          ...
          <div className="lg:col-span-4 bg-[#0F1528] border border-[#1F2947] p-5 rounded-3xl shadow-sm relative overflow-hidden h-full flex flex-col justify-between font-sans">
            <div className="space-y-2">
              <div className="flex items-center justify-between">
            ...
            <div className="mt-4 pt-4 border-t border-slate-50 dark:border-slate-855 font-sans">
              <div className="flex justify-between text-[11px] text-slate-500 mb-2 font-sans font-medium">
                ...
                <span className="font-bold text-slate-200">{completedTasks}/{totalTasks}</span>
              </div>
              <button 
                id="nav-to-tasks"
          ...
          <div className="lg:col-span-4 bg-[#0F1528] border border-[#1F2947] p-5 rounded-3xl shadow-sm relative overflow-hidden h-full flex flex-col justify-between font-sans">
            ...
            <div className="mt-4 pt-4 border-t border-slate-50 dark:border-slate-855">
              <div className="flex justify-between text-[11px] text-slate-505 mb-2 font-sans font-medium">
                ...
                <span className="font-bold text-slate-200">{unlockedBadges.length}/{badges.length}</span>
              </div>
              <button 
          ...
          <div className="lg:col-span-12 bg-[#0F1528] border border-[#1F2947] p-6 rounded-3xl shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-6 relative overflow-hidden">
            ...
            <div className="space-y-3 max-w-lg">
              <div className="flex items-center gap-2">
                ...
                <h4 className="font-display font-extrabold text-slate-100">Weekly Founder Audit Streak</h4>
              </div>
                ...
                return (
                  <div key={idx} className="flex flex-col items-center">
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-xs font-semibold border transition-all ${
                      ...
                      'bg-[#0B0F19]/60 text-slate-400 border-[#1F2947] dark:border-slate-800'
                    }`}>
                      {day[0]}
                    </div>
