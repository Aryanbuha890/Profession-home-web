...
export const Route = createFileRoute("/app/startup/vault")({
  component: AchievementVaultWrapper,
});
  ...
  return (
    <div id="achievement-vault" className="space-y-8 max-w-6xl mx-auto">
      ...
      <div className="space-y-4">
        <h3 className="font-display font-extrabold text-sm tracking-wide uppercase text-slate-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {badges.map((badge) => (
            <div
              ...
              className={`bg-[#0F1528] border rounded-2xl p-5 hover:scale-[1.01] hover:shadow-md transition-all cursor-pointer relative overflow-hidden flex flex-col justify-between space-y-4 ${
                badge.unlocked 
            ...
            >
              <div className="space-y-3">
                <div className="flex justify-between items-start">
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-2xl border ${
                    badge.unlocked 
                      ...
                      : 'bg-[#0B0F19] dark:bg-slate-900 border-[#1F2947] grayscale'
                  }`}>
                ...
                <div className="space-y-1">
                  <h4 className="text-xs font-bold text-slate-850 text-white flex items-center gap-1.5">
                    {badge.title}
                    {badge.unlocked && (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-4">
        ...
        {/* Certificate Card */}
        <div className="bg-[#0F1528] border border-[#1F2947] p-6 rounded-3xl relative overflow-hidden flex flex-col justify-between shadow-sm">
          <div className="space-y-3">
          ...
          <div className="mt-4 pt-4 border-t border-[#1F2947]">
            <button
              ...
              className={`w-full py-2.5 rounded-xl text-xs font-semibold flex items-center justify-center gap-1.5 transition-all shadow-sm ${
                unlockedBadges.length >= 4
                  ...
                  : 'bg-slate-105 select-none opacity-50 cursor-not-allowed border text-slate-400 border-[#1F2947]'
              }`}
            >
...
function AchievementVaultWrapper() {
  const context = useStartupContext();
  return <AchievementVault {...context} />;
}
