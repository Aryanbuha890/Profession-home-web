...
export const Route = createFileRoute("/app/investor/startup_discovery")({
  component: StartupDiscoveryPage,
});

function StartupDiscoveryPage() {
  return (
    <div className="space-y-6 animate-in fade-in zoom-in duration-500 p-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-white">Startup Discovery</h1>
          <p className="text-sm text-slate-400 mt-1">Manage startup discovery settings and view data.</p>
        </div>
      ...
      <div className="bg-slate-900/40 border border-slate-800/60 rounded-2xl p-6 min-h-[400px]">
        ...
        <div className="text-center text-slate-500 mt-20">
          <p>No startup discovery found yet.</p>
        </div>
      </div>
    </div>
  );
