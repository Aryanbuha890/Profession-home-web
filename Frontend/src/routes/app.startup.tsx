...
import { 
  ...
  Users, Briefcase, DollarSign, FileText, UsersRound, ShoppingBag, TrendingUp, Trophy, Award, MessageSquare, Activity, Sparkles, Target, Zap
} from "lucide-react";
...
import { 
  ...
  TaskStatus,
  TaskPriority
} from './app.startup/-types';
import { StartupContext } from './app.startup/-context';
...
const NAV_ITEMS = [
  ...
  { icon: Activity, label: 'Startup Diagnostic', to: '/app/startup/assessment' },
  { icon: Map, label: 'Roadmap Progress', to: '/app/startup/roadmap' },
  { icon: CheckSquare, label: 'Sprint Board', to: '/app/startup/tasks' },
  { icon: FileText, label: 'Document Package', to: '/app/startup/documents' },
  { icon: Sparkles, label: 'Gemini Copilot', to: '/app/startup/ai' },
  { icon: Award, label: 'Ecosystem Vault', to: '/app/startup/vault' },
  { icon: Users, label: 'Expert Mentors', to: '/app/startup/mentors' },
  { icon: MessageSquare, label: 'Networking Feed', to: '/app/startup/community' },
];
    ...
    <div className="flex h-full min-h-[calc(100vh-80px)] w-full bg-[#0B0F19] text-slate-200 overflow-hidden font-sans selection:bg-indigo-500/30 selection:text-indigo-200">
      ...
      <div className="flex-1 flex flex-col h-full min-w-0 bg-[#0B0F19]">
        <header className="h-20 flex-shrink-0 flex items-center justify-between px-6 lg:px-10 border-b border-[#1F2947] backdrop-blur-md z-10">
          ...
          {/* Top level assessment badge */}
          <div className="flex items-center gap-4">
            <div className="bg-[#0F1528] border border-[#1F2947] px-3 py-1.5 rounded-xl flex items-center gap-2">
              <div className="w-2.5 h-2.5 rounded-full bg-emerald-500 animate-pulse"></div>
              <span className="text-[10px] font-mono font-bold uppercase text-slate-400">Startup level: {Math.floor(assessment.overallScore / 20) + 1}</span>
            </div>
          </div>
        </header>
