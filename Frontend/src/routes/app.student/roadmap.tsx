...
import { ArrowLeft, Bell, Calendar, ChevronLeft, ChevronRight, Edit3, HelpCircle, Mail, Phone, Plus, Star, User, UserCheck, X, Check, Play } from "lucide-react";

export const Route = createFileRoute("/app/student/roadmap")({
...
function CareerRoadmapPage() {
  ...
  return (
    <div className="min-h-screen bg-[#05060F] text-slate-100 p-6 md:p-8 space-y-6 animate-in fade-in duration-750">
      
      {/* 1. Header Navigation Bar (Adapted from header in Patient card) */}
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 border-b border-white/5 pb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-white font-display">Career Roadmap</h1>
            <p className="text-slate-500 text-xs mt-0.5">Software Engineering Path Tracking</p>
          </div>
        </div>

        <div className="flex items-center gap-3 self-end sm:self-auto">
          {/* Path history dropdown link */}
      ...
      {/* Main Grid: Body Layout + Sidebar Panel */}
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 items-start">
        
        {/* Left 3 Columns: Profile header & calendar schedule */}
        <div className="lg:col-span-3 space-y-6">
          
          {/* 2. Path Profile Card (Adapted from Irene Fleming profile card) */}
          <div className="rounded-2xl border border-white/5 bg-slate-900/40 p-6 flex flex-col md:flex-row gap-6 justify-between shadow-[0_4px_35px_rgba(0,0,0,0.5)]">
            ...
            {/* Middle Stats - Phone, email, target details */}
          ...
          {/* 3. Weekly Schedule Timeline Grid (Adapted from Patient appointment schedule) */}
          <div className="rounded-2xl border border-white/5 bg-slate-900/20 p-6 space-y-4">
            
            {/* Week Header row (Sunday 16 - Saturday 22) */}
            ...
            <div className="relative min-h-[420px] pt-2">
              ...
              {/* Horizontal current-time progress marker line (at 8:45 AM) */}
        ...
        {/* Right 1 Column: Mini-Calendar, Mentors assigned, Recommended actions */}
        <div className="space-y-6">
          
          {/* 4. Mini Calendar Widget (Adapted from Calendar sidebar widget) */}
          <div className="rounded-2xl border border-white/5 bg-slate-900/40 p-4 space-y-3 shadow-md">
            ...
            <div className="grid grid-cols-7 gap-1 text-[9px] text-center font-bold text-slate-500">
              ...
              {/* Calendar Days items - Mock values matching June 2026 calendar */}
              {Array.from({ length: 31 }).map((_, i) => {
                const dateNum = i + 1;
                const isSelected = dateNum === selectedCalendarDay;
                // Offset by 1 for Monday start
                return (
                  <button
                    key={`cal-day-${dateNum}`}
          ...
          {/* 5. Doctors Panel List ➔ Mentors Panel (Adapted from Doctors list in sidebar) */}
          ...
          {/* 6. Write Prescription card ➔ Goal Recommended Actions checklist (Adapted from bottom sidebar card) */}
      ...
      {/* NEW TIMELINE BLOCK CREATOR MODAL */}
      {isModalOpen && (
