...
function PitchDeckDocuments({
  ...
  return (
    <div id="pitch-deck-documents" className="space-y-8 max-w-7xl mx-auto px-1">
      ...
      {/* Visual Header */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-[#0F1528] border border-[#1F2947] p-6 rounded-3xl shadow-sm">
        ...
        <div className="flex gap-4">
          <div className="bg-[#0B0F19] border border-[#1F2947] px-4 py-3 rounded-2xl flex items-center gap-2.5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-4 bg-[#0F1528] border border-[#1F2947] p-6 rounded-3xl shadow-sm flex flex-col justify-between h-full space-y-6">
          <div className="space-y-4">
            ...
            <div className="space-y-1">
              ...
              <div className="w-full bg-[#1F2947] h-2 rounded-full overflow-hidden relative">
                <div
            ...
            <div className="space-y-2.5 pt-2">
              {documents.map((doc) => {
                ...
                return (
                  <div
                    key={doc.id}
                    className={`flex items-center justify-between border border-dashed rounded-xl p-3 transition-all duration-200 ${isUploaded
                        ? 'border-emerald-500/10 bg-emerald-500/5 text-emerald-950 dark:text-emerald-100'
                        : 'border-[#1F2947] text-slate-500'
                      }`}
                  >
                    ...
                    {isUploaded ? (
                      ...
                      <button
                        ...
                        className="text-[9px] font-mono font-extrabold uppercase bg-[#0B0F19] hover:bg-indigo-50 hover:text-indigo-600 dark:bg-slate-800 text-slate-600 dark:text-slate-300 px-2 py-1 rounded-md transition-all shrink-0 border border-[#1F2947] dark:border-slate-700 hover:border-indigo-200"
                      >
                        Mock Upload
                      </button>
                    )}
          ...
          <div className="pt-4 border-t border-[#1F2947] text-xs text-slate-500">
        ...
        <div className="lg:col-span-8 bg-[#0F1528] border border-[#1F2947] p-6 rounded-3xl shadow-sm space-y-6">
          <div className="flex justify-between items-center pb-3 border-b border-slate-50 dark:border-slate-850">
            <h3 className="font-display font-black text-slate-100 text-lg">
              Virtual Repository
            </h3>
            ...
            {documents.map((doc) => {
              ...
              return (
                <div
                  ...
                  className={`border rounded-2xl p-4 flex flex-col justify-between transition-all duration-300 relative overflow-hidden group ${isSelected
                      ? 'border-indigo-650 bg-indigo-50/10 dark:bg-indigo-950/10 shadow-sm ring-1 ring-indigo-600/15'
                      : dragOverDocId === doc.id
                        ? 'border-indigo-500 bg-indigo-500/5 scale-[1.01]'
                        : 'border-[#1F2947] hover:border-[#1F2947] hover:-translate-y-0.5'
                    }`}
                >
                  <div className="flex justify-between items-start mb-3">
                    <div className="flex gap-2.5 items-center">
                      <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 transition-colors ${isUploaded
                  ...
                  {isUploaded ? (
                      ...
                      <div className="flex gap-2">
                        {doc.score === undefined ? (
                          ...
                          <button
                            ...
                            className="font-bold px-3 py-1.5 rounded-xl text-[10px] border border-[#1F2947] hover:bg-[#1F2947]  transition-colors inline-flex items-center gap-1"
                          >
                    ...
                    <button
                      ...
                      onClick={() => handleSimulatedUpload(doc.id)}
                      className="w-full mt-2 py-2.5 border border-dashed border-[#1F2947] rounded-xl text-center hover:bg-[#1F2947]  transition-all text-[10px] text-slate-550 dark:text-slate-400 font-semibold flex items-center justify-center gap-1"
                    >
      ...
      {activeDocForFeedback && activeDocForFeedback.slideFeedback && !analyzingDocId && (
        <motion.div
          ...
          className="bg-[#0F1528] border border-[#1F2947] p-6 rounded-3xl shadow-sm space-y-6"
        >
          {/* Header */}
          <div className="border-b border-slate-50 dark:border-slate-850 pb-5 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
              ...
              <h3 className="text-2xl font-display font-extrabold text-slate-850 text-white">
          ...
          <div className="flex gap-3 bg-[#0B0F19]/50  border border-[#1F2947] p-4 rounded-2xl items-start">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 pt-2">
              ...
              {activeDocForFeedback.slideFeedback.map((slide, idx) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left p-3 rounded-2xl transition-all border flex items-center justify-between gap-2 block ${isActive
                        ? 'bg-slate-900 border-slate-950 text-white shadow dark:bg-white dark:border-white dark:text-slate-900 font-semibold'
                        : 'bg-[#0B0F19]/30 border-[#1F2947]  dark:border-slate-850 hover:bg-[#1F2947] hover:border-[#1F2947]'
                      }`}
                  >
                    <div className="truncate space-y-0.5">
                      <span className={`text-[9px] font-mono tracking-widest uppercase block ${isActive ? 'text-indigo-400 dark:text-indigo-650' : 'text-slate-400'
                        }`}>
                        Slide {slide.slide}
                      </span>
            ...
            <div className="lg:col-span-5 space-y-3">
              ...
              <p className="text-[9.5px] text-slate-400 font-mono text-center flex items-center justify-center gap-1.5 bg-[#0B0F19]/40 p-2 rounded-lg">
            ...
            <div className="lg:col-span-4 bg-[#0B0F19]/30  border border-[#1F2947] rounded-2xl p-4 flex flex-col justify-between h-full space-y-4">
              {activeDocForFeedback.slideFeedback[selectedSlideIdx] ? (
                <div className="space-y-3">
                  <div className="flex justify-between items-center bg-[#0F1528] p-2.5 rounded-xl border border-slate-150 dark:border-slate-800 shadow-sm">
...
function PitchDeckDocumentsWrapper() {
  const context = useStartupContext();
  return <PitchDeckDocuments {...context} />;
}

export default PitchDeckDocumentsWrapper;
