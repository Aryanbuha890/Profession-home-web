...
function AICopilot({
  ...
  const getLatestActionPlan = (): string[] => {
    for (let i = messages.length - 1; i >= 0; i--) {
      if (messages[i].sender === 'assistant' && messages[i].actionPlan) {
        return messages[i].actionPlan as string[];
      }
    }
    return [
  ...
  return (
    <div id="ai-copilot" className="grid grid-cols-1 lg:grid-cols-12 gap-8 max-w-6xl mx-auto h-[600px] md:h-[680px]">
      ...
      <div className="lg:col-span-8 bg-[#0F1528] border border-slate-205 border-[#1F2947] rounded-3xl overflow-hidden shadow-sm flex flex-col h-full justify-between">
        ...
        <div className="flex-1 p-5 overflow-y-auto space-y-5">
            ...
            return (
              ...
              >
                <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 text-xs font-bold border font-mono ${
                  isUser ? 'bg-indigo-600 border-indigo-600 text-white' : 'bg-[#0B0F19] dark:bg-slate-950 border-[#1F2947] text-indigo-500'
                }`}>
                  {isUser ? 'F' : 'G'}
                </div>
                ...
                <div className={`space-y-2 p-4 rounded-2xl border text-xs leading-relaxed font-sans ${
                  isUser 
                    ...
                    : 'bg-[#0B0F19] border-[#1F2947]  dark:border-slate-850 text-slate-800 dark:text-slate-205'
                }`}>
                  <p className="whitespace-pre-line font-medium leading-relaxed">{msg.text}</p>
                  
                  {!isUser && (
                    <div className="flex justify-end pt-2 border-t border-[#1F2947]/60 text-[10px] text-slate-400">
                      <button 
                        id={`copy-chat-${idx}`}
          ...
          {loading && (
            <div className="flex gap-3 mr-auto max-w-[80%] items-center">
              <div className="w-8 h-8 rounded-full bg-[#0B0F19] dark:bg-slate-950 border border-[#1F2947] text-indigo-500 flex items-center justify-center font-bold text-xs">
                G
              </div>
              <div className="bg-[#0B0F19]/60 border border-[#1F2947] p-4 rounded-2xl flex items-center gap-2">
        ...
        <div className="p-4 border-t border-[#1F2947] dark:border-slate-850/80 space-y-4 shrink-0 bg-[#0B0F19]/40 ">
          ...
          <div className="flex gap-2 pb-1 overflow-x-auto select-none no-scrollbar">
            {suggestedPrompts.map((prompt, idx) => (
              <button
                ...
                className="bg-[#0F1528] border border-[#1F2947] px-3 py-1.5 rounded-xl text-[10px] font-semibold text-slate-600 dark:text-slate-350 hover:bg-[#1F2947]  shrink-0 select-none transition-all"
              >
                {prompt}
              </button>
            ))}
          ...
          >
            <input
              ...
              className="flex-1 text-xs px-4 py-3 rounded-2xl border border-slate-205 border-[#1F2947] bg-[#0F1528]/60 shadow-sm"
            />
            <button
              id="chat-submit"
              type="submit"
      ...
      <div className="lg:col-span-4 bg-[#0F1528] border border-slate-205 border-[#1F2947] p-5 rounded-3xl h-full flex flex-col justify-between shadow-sm overflow-y-auto">
        <div className="space-y-4">
          <div className="flex items-center gap-2 border-b border-slate-50 dark:border-slate-900 pb-3">
            <TrendingUp className="w-5 h-5 text-indigo-500" />
            <h3 className="font-display font-extrabold text-xs tracking-wide uppercase text-white">Active Growth Action Plan</h3>
          </div>
          ...
          <div className="space-y-3 pt-2">
            {currentActionPlan.map((action, idx) => (
              <div 
                key={idx}
                className="bg-[#0B0F19]/50  p-3.5 border border-[#1F2947] rounded-2xl flex gap-3 hover:translate-x-0.5 transition-transform"
              >
                ...
                <p className="text-[11px] text-slate-300 leading-normal font-sans font-medium">{action}</p>
              </div>
            ))}
          </div>
        </div>

        <div className="mt-6 pt-4 border-t border-[#1F2947] space-y-1.5 text-[10px] text-slate-400">
...
function AICopilotWrapper() {
  ...
  return <AICopilot {...context} />;
}

export default AICopilotWrapper;
