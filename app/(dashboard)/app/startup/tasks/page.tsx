    ...
    <div id="tasks-execution" className="space-y-8 max-w-6xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          {/* Sub Views Switches */}
          <div className="bg-[#0B0F19] dark:bg-slate-900 p-1 rounded-xl flex border border-[#1F2947]">
            <button 
              id="view-kanban"
              onClick={() => setActiveView('kanban')}
              className={`p-1.5 rounded-lg text-xs font-semibold flex items-center gap-1 ${
                activeView === 'kanban' ? 'bg-[#0F1528] shadow-sm font-bold text-slate-850 text-white' : 'text-slate-500'
              }`}
            >
            ...
            <button 
              ...
              className={`p-1.5 rounded-lg text-xs font-semibold flex items-center gap-1 ${
                activeView === 'matrix' ? 'bg-[#0F1528] shadow-sm font-bold text-slate-850 text-white' : 'text-slate-500'
              }`}
            >
            ...
            <button 
              ...
              className={`p-1.5 rounded-lg text-xs font-semibold flex items-center gap-1 ${
                activeView === 'table' ? 'bg-[#0F1528] shadow-sm font-bold text-slate-850 text-white' : 'text-slate-500'
              }`}
            >
      ...
      <div className="flex border-b border-[#1F2947]">
      ...
      <AnimatePresence mode="wait">
        {activeView === 'kanban' && (
          ...
          >
            {statuses.map((status) => {
              ...
              return (
                <div key={status} className="bg-[#0B0F19]/55  p-4 rounded-3xl border border-[#1F2947] flex flex-col min-h-[500px]">
                  ...
                  <div className="space-y-4 flex-1 overflow-y-auto max-h-[600px] pr-1">
                    {statusTasks.length === 0 ? (
                      <div className="text-center py-10 border border-dashed border-[#1F2947] rounded-2xl text-xs text-slate-400">
                        No tasks here
                      </div>
                    ) : (
                      statusTasks.map((task) => (
                        <div
                          ...
                          className="bg-[#0F1528] border border-slate-150 dark:border-slate-850 p-4 rounded-2xl shadow-sm hover:shadow-md hover:border-indigo-600/20 hover:scale-[1.01] transition-all cursor-pointer space-y-3"
                        >
                          ...
                          <p className="text-[11px] text-slate-400 line-clamp-2 font-sans font-normal">
                            {task.description}
                          </p>
                          ...
                          <div className="flex justify-between items-center pt-2.5 border-t border-[#1F2947]/80 text-[10px] text-slate-400 font-medium">
            ...
            <div className="bg-rose-500/5 border border-rose-500/15 p-6 rounded-3xl space-y-4">
              <div className="flex justify-between items-center border-b border-rose-500/10 pb-3">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="font-display font-extrabold text-sm text-white">Q1: Immediate Execution</h3>
                </div>
              ...
              <div className="space-y-3 max-h-[300px] overflow-y-auto">
                {filteredTasks.filter(t => t.priority === 'High' && t.status !== 'Completed').map(task => (
                  <div key={task.id} onClick={() => setSelectedTask(task)} className="bg-[#0F1528] border border-[#1F2947] p-3.5 rounded-xl flex justify-between items-center cursor-pointer hover:translate-x-0.5 transition-transform shadow-sm">
            ...
            <div className="bg-indigo-55/5 border border-indigo-500/15 p-6 rounded-3xl space-y-4">
              <div className="flex justify-between items-center border-b border-indigo-500/10 pb-3">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="font-display font-extrabold text-sm text-white">Q2: Strategic Development</h3>
                </div>
            ...
            <div className="bg-amber-500/5 border border-amber-500/15 p-6 rounded-3xl space-y-4">
              <div className="flex justify-between items-center border-b border-amber-500/10 pb-3">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="font-display font-extrabold text-sm text-white">Q3: Delegation & Support</h3>
                </div>
            ...
            <div className="bg-emerald-500/5 border border-emerald-500/15 p-6 rounded-3xl space-y-4">
              <div className="flex justify-between items-center border-b border-emerald-500/10 pb-3">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="font-display font-extrabold text-sm text-white">Q4: Archival Done Items</h3>
                </div>
                <span className="text-[10px] font-mono text-emerald-500 uppercase font-bold">Safe Accomplished Blocks</span>
              </div>
              <div className="space-y-3 max-h-[300px] overflow-y-auto">
                {filteredTasks.filter(t => t.status === 'Completed').map(task => (
                  <div key={task.id} onClick={() => setSelectedTask(task)} className="bg-[#0F1528] border border-[#1F2947] p-3.5 rounded-xl flex justify-between items-center cursor-pointer hover:translate-x-0.5 transition-transform shadow-sm opacity-60">
        ...
        {activeView === 'table' && (
          ...
          <motion.div 
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="bg-[#0F1528] border border-[#1F2947] rounded-2xl overflow-hidden shadow-sm"
          >
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse text-xs">
                <thead>
                  <tr className="bg-[#0B0F19] text-slate-400 font-mono text-[10px] uppercase border-b border-slate-150 dark:border-slate-800">
                ...
                <tbody>
                  {filteredTasks.map(task => (
                    <tr key={task.id} className="border-b last:border-0 border-[#1F2947] hover:bg-[#1F2947]/50">
                      ...
                      <td className="p-4">
                        <select
                          ...
                          className="bg-[#0B0F19] dark:bg-slate-900 border border-[#1F2947] p-1.5 rounded-lg text-xs"
                        >
        ...
        {modalOpen && (
          <div className="fixed inset-0 bg-slate-950/40 backdrop-blur-xs flex items-center justify-center p-4 z-50">
            <motion.div 
              ...
              className="bg-[#0F1528] border border-slate-250 border-[#1F2947] p-6 rounded-3xl max-w-md w-full shadow-xl space-y-6"
            >
              <div className="flex justify-between items-center border-b border-[#1F2947] pb-3">
                <h3 className="font-display font-extrabold text-sm text-white">Create Sprint Log</h3>
              ...
              <form onSubmit={handleCreateTask} className="space-y-4">
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    className="w-full text-xs p-3 rounded-xl border border-[#1F2947] bg-[#0B0F19]"
                  />
                </div>
                ...
                <div className="space-y-1">
                  ...
                  <textarea
                    ...
                    className="w-full text-xs p-3 rounded-xl border border-[#1F2947] bg-[#0B0F19] h-20 resize-none"
                  />
                </div>
        ...
        {selectedTask && (
          <div className="fixed inset-0 bg-slate-950/40 backdrop-blur-xs flex items-center justify-center p-4 z-50">
            <motion.div
              ...
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.95, opacity: 0 }}
              className="bg-[#0F1528] border border-slate-250 border-[#1F2947] p-6 rounded-3xl max-w-xl w-full shadow-2xl space-y-6"
            >
              <div className="flex justify-between items-start gap-4">
                <div className="space-y-1 flex-1">
                  ...
                  <h3 className="text-lg font-display font-black text-white leading-snug">{selectedTask.title}</h3>
                </div>
              ...
              {/* Status control */}
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 bg-[#0B0F19]/60 p-4 rounded-2xl border border-[#1F2947]">
                <div className="space-y-1">
                  ...
                  <select
                    ...
                    className="w-full bg-[#0F1528] border border-[#1F2947] p-1 rounded-xl text-xs font-semibold"
                  >
                    <option value="To Do">To Do</option>
                    <option value="In Progress">In Progress</option>
                    <option value="Review">Review</option>
                ...
                <div className="space-y-0.5 text-xs">
                  ...
                  <span className="font-bold text-slate-200 block pt-1.5 leading-none">{selectedTask.priority}</span>
                </div>

                <div className="space-y-0.5 text-xs">
                  <span className="text-[9px] font-mono text-slate-400 uppercase block">Assignee</span>
                  <span className="font-bold text-slate-200 block pt-1.5 leading-none">{selectedTask.assignedTo}</span>
                </div>
              ...
              <div className="space-y-3">
                ...
                <div className="bg-gradient-to-tr from-slate-50 to-indigo-50/20 dark:from-slate-900/40 dark:to-indigo-950/20 border border-indigo-500/30 p-4 rounded-2xl relative overflow-hidden">
              ...
              <div className="pt-4 border-t border-[#1F2947]/80 flex justify-end">
                <button 
...
function TasksExecutionWrapper() {
  const context = useStartupContext();
  return <TasksExecution {...context} />;
}

export default TasksExecutionWrapper;
