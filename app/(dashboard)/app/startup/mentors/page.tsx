...
function MentorSection({ overallScore }: MentorSectionProps) {
  ...
  return (
    <div id="mentor-section" className="space-y-8 max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {MOCK_MENTORS.map((mentor) => (
            <div
              ...
              className="bg-[#0F1528] border border-[#1F2947] p-5 rounded-2xl hover:shadow-lg transition-all flex flex-col justify-between space-y-5"
            >
              <div className="space-y-4">
                ...
                <div className="flex gap-4 items-start">
                  <img 
                    ...
                    src={mentor.photoUrl} 
                    alt={mentor.name} 
                    className="w-14 h-14 rounded-2xl object-cover shrink-0 border border-[#1F2947]"
                  />
                  <div className="space-y-0.5 truncate">
                    <h4 className="text-xs font-bold text-slate-850 text-white truncate">{mentor.name}</h4>
                ...
                <div className="flex flex-wrap gap-1.5 pt-1">
                  {mentor.tags.slice(0, 3).map((tag, idx) => (
                    <span key={idx} className="text-[9px] bg-[#0B0F19] dark:bg-slate-900 border border-[#1F2947] text-slate-400 px-2.5 py-0.5 rounded">
                      {tag}
                    </span>
                  ))}
                </div>
      ...
      <div className="space-y-4 pt-4 border-t border-[#1F2947]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {STARTUP_BLOGS.map((blog) => (
            <div
              key={blog.id}
              className="bg-[#0F1528] border border-slate-150 dark:border-slate-850 rounded-2xl overflow-hidden shadow-sm flex flex-col h-full hover:shadow-md transition-all hover:scale-[1.01]"
            >
              <img 
                referrerPolicy="no-referrer"
                src={blog.imageUrl} 
              ...
              <div className="p-4 flex flex-col justify-between flex-1 space-y-3">
                <div className="space-y-1.5">
                  ...
                  <h4 className="text-xs font-bold text-slate-850 text-white leading-normal h-8 line-clamp-2">
                    {blog.title}
                  </h4>
        ...
        {selectedMentor && (
          <div className="fixed inset-0 bg-slate-950/40 backdrop-blur-xs flex items-center justify-center p-4 z-50">
            <motion.div
              ...
              className="bg-[#0F1528] border border-slate-250 border-[#1F2947] p-6 rounded-3xl max-w-md w-full shadow-2xl relative overflow-hidden space-y-6"
            >
              <div className="flex justify-between items-start border-b border-[#1F2947] pb-3">
                <div>
                  <span className="text-[9px] font-mono text-indigo-500 uppercase tracking-widest block font-bold">Redeem Mentorship Token</span>
                  <h3 className="text-sm font-bold text-slate-850 text-white">Schedule Advisor Session</h3>
                </div>
              ...
              {bookingConfirmation ? (
                ...
                <div className="space-y-5">
                  <div className="flex items-center gap-3 bg-[#0B0F19] p-3 rounded-2xl">
                    <img 
                      referrerPolicy="no-referrer"
                  ...
                  <div className="space-y-2">
                    ...
                    <button
                      ...
                      className="w-full text-left p-3.5 rounded-xl border border-[#1F2947] bg-[#0F1528] text-xs font-semibold flex justify-between items-center hover:bg-[#1F2947] transition-colors"
                    >
                      <div className="space-y-0.5">
                        <span className="text-slate-300 font-bold block">Direct Premium Call</span>
...
function MentorSectionWrapper() {
  const context = useStartupContext();
  return <MentorSection {...context} />;
}

export default MentorSectionWrapper;
