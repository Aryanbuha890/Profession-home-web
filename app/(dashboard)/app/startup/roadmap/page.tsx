...
function StartupRoadmap({ phases, onToggleMilestone }: StartupRoadmapProps) {
  ...
  return (
    <div id="startup-roadmap" className="space-y-8 max-w-5xl mx-auto">
      ...
      {recommended && (
        <div className="bg-[#0F1528] border border-indigo-500/30 p-4 rounded-2xl flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div className="flex items-start gap-3">
            <div className="w-10 h-10 rounded-xl bg-indigo-500/10 text-indigo-400 flex items-center justify-center shrink-0 mt-0.5">
              <Sparkles className="w-5 h-5 animate-pulse" />
            </div>
            <div className="space-y-1">
              <span className="text-[10px] font-mono text-indigo-400 font-bold tracking-wider uppercase">Recommended Next Milestone</span>
              <h4 className="text-sm font-semibold text-white">{recommended.milestone.title}</h4>
              <p className="text-xs text-slate-400 max-w-2xl">{recommended.milestone.description}</p>
            </div>
          </div>
          <span className="text-xs font-mono font-bold bg-indigo-500/10 px-3 py-1.5 rounded-xl text-indigo-400">
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
        ...
        <div className="md:col-span-4 bg-[#0F1528] border border-[#1F2947] p-5 rounded-2xl space-y-4 shadow-sm flex flex-col justify-between">
          <div className="space-y-1">
          ...
          <div className="bg-[#0B0F19] p-4 rounded-xl space-y-3 border border-[#1F2947]">
        ...
        <div className="md:col-span-8 bg-[#0F1528] border border-[#1F2947] p-5 rounded-2xl shadow-sm flex flex-col justify-between">
          <div className="space-y-1">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 pt-3">
            <div className="bg-[#0B0F19]/60 p-3.5 rounded-xl border border-[#1F2947]/60 space-y-1.5">
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-xl capitalize border text-[11px] transition-all font-semibold ${
                  filterMode === mode 
                    ...
                    : 'bg-[#0F1528] border-[#1F2947] text-slate-400 hover:bg-[#1F2947]'
                }`}
              >
                {mode}
              </button>
        ...
        {phases.map((phase) => {
          ...
          return (
            <div 
              ...
              className={`bg-[#0F1528] border rounded-2xl overflow-hidden transition-all ${
                isExpanded 
                  ? 'border-indigo-600/30 shadow-md ring-1 ring-indigo-500/10' 
                  : 'border-[#1F2947] shadow-sm'
              }`}
            >
              {/* Header section clickable to expand/collapse */}
              <div 
                onClick={() => handleToggleExpand(phase.id)}
                className="p-5 flex items-center justify-between cursor-pointer hover:bg-[#1F2947]/40  select-none"
              >
                <div className="flex items-center gap-4 shrink-0">
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center border transition-all ${
                    ...
                    'bg-[#0F1528] border-[#1F2947] text-slate-500'
                  }`}>
                    <span className="text-xs font-bold font-mono">{phase.id}</span>
                  </div>
                  <div>
                    <h3 className="font-display font-extrabold text-sm text-white">{phase.phaseName}</h3>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="hidden md:flex flex-col items-end gap-1 shrink-0">
                    ...
                    <div className="w-24 bg-[#1F2947] h-1.5 rounded-full overflow-hidden">
              ...
              <AnimatePresence initial={false}>
                {isExpanded && (
                  <motion.div
                    ...
                    animate={{ height: "auto" }}
                    exit={{ height: 0 }}
                    className="overflow-hidden border-t border-[#1F2947] bg-[#0B0F19]/30 "
                  >
                      ...
                      {phase.milestones.map((milestone) => (
                        <div 
                          ...
                          className={`flex items-start gap-4 p-4 rounded-xl border transition-all ${
                            milestone.completed 
                              ? 'bg-emerald-500/5 dark:bg-emerald-500/10 border-emerald-500/20 text-slate-300 cursor-pointer' 
                              : milestone.locked 
                                ? 'bg-[#0B0F19]/60 border-[#1F2947] opacity-50 cursor-not-allowed select-none' 
                                : 'bg-[#0F1528] border-[#1F2947] hover:border-indigo-600/30 hover:shadow-sm cursor-pointer hover:translate-x-0.5'
                          }`}
                        >
                          <div className="shrink-0 mt-0.5">
                            {milestone.completed ? (
                              ...
                              <div className="w-6 h-6 rounded-full bg-[#1F2947] border border-[#1F2947] text-slate-400 flex items-center justify-center">
                                <Lock className="w-3.5 h-3.5" />
                              </div>
                            ) : (
                              <div className="w-6 h-6 rounded-full border-2 border-[#1F2947] hover:border-indigo-500 flex items-center justify-center bg-[#0F1528]">
                          ...
                          <div className="space-y-1">
                            <div className="flex items-center gap-2">
                              <h4 className={`text-xs font-bold leading-none ${milestone.completed ? 'line-through text-slate-400' : 'text-white'}`}>
                                {milestone.title}
                              </h4>
                              {milestone.locked && (
                                <span className="text-[8px] font-mono font-bold uppercase bg-[#1F2947] px-1.5 py-0.5 rounded text-slate-500">Locked</span>
                              )}
                            </div>
...
function StartupRoadmapWrapper() {
  const context = useStartupContext();
  return <StartupRoadmap {...context} />;
}

export default StartupRoadmapWrapper;
